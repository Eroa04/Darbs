        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    <h2 style="margin-left:3%;">Registrar Actividad</h2>
                    <div style="width:50%; padding:2%; margin:2% auto 3% auto; border:1px solid gray; border-radius:6px; background-color:white;">
                        <form action="{{url('actividades')}}" method="POST" enctype="multipart/form-data" >
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group col-md-12">
                                    Descripcion:
                                    <textarea class="form-control" id="descripcion" name="descripcion" placeholder="Descripcion" value="{{old('descripcion')}}"></textarea>
                                    @if(count($errors)>0)
                                    <p style="padding:2px; color:red;">{{$errors->first('descripcion')}}</p>
                                    @endif
                                </div>
                                <div class="form-group col-md-6">
                                    Fecha Inicio:
                                    <input type="text" onfocus="(this.type='date')" onblur="(this.type='text')" class="form-control" id="fechainicio" name="fechainicio" placeholder="Fecha Inicio" value="{{old('fechainicio')}}">
                                    @if(count($errors)>0)
                                    <p style="padding:2px; color:red;">{{$errors->first('fechainicio')}}</p>
                                    @endif
                                </div>
                                <div class="form-group col-md-6">
                                    Fecha Fin:
                                    <input type="text" onfocus="(this.type='date')" onblur="(this.type='text')" class="form-control" id="fechafin" name="fechafin" placeholder="Fecha Fin" value="{{old('fechafin')}}">
                                    @if(count($errors)>0)
                                    <p style="padding:2px; color:red;">{{$errors->first('fechafin')}}</p>
                                    @endif
                                </div>
                                <div class="form-group col-md-12">
                                    Estado:
                                    <select class="form-control" id="estado" name="estado" style="color:grey;">
                                        ...
                                        <option value="Terminada" {{ old('estado') == "Terminada" ? 'selected' : '' }}>Terminada</option>
                                    </select>
                                    @if(count($errors)>0)
                                ...
                                <div class="form-group col-md-12">
                                    Obra:
                                    <select class="form-control" id="idobra" name="idobra" style="color:grey;">
                                        <option value="">-- Asignar obra --</option>
                                        @foreach ($obras as $obra)
                                        <option value="{{$obra->IdObra}}" >{{$obra->Nombre}}</option>
                                        @endforeach
                                    </select>
                                    @if(count($errors)>0)
                                    <p style="padding:2px; color:red;">{{$errors->first('idobra')}}</p>
                                    @endif
                                </div>
                            </div>
